#!/usr/bin/env python3
"""
Iris Logistic Classification Example - Python
Equivalent to iris_logistic_classification.v
...
def main():
    print_section("Python: Iris Logistic Classification Example")
    
    # 1. Load Iris dataset
    print_subsection("1. LOADING IRIS DATASET")
    ...
    print(f"Target name: {iris.target_name}")
    ...
    # 3. Create binary classification
    print_subsection("3. PREPARING BINARY CLASSIFICATION")
    y_train_binary = (y_train == 0).astype(int)
    y_test_binary = (y_test == 0).astype(int)
    print("Binary labels created (Class 0 = 1, Others = 0)")
    print()
    
    # 4. Train logistic regression
    print_subsection("4. TRAINING LOGISTIC REGRESSION MODEL")
    model = LogisticRegression(max_iter=1000, learning_rate_init=0.01)
    model.fit(X_train, y_train_binary)
    print(f"Model trained with max_iter=1000")
    ...
    for i in range(min(10, len(y_train_pred))):
        pred_label = "Class 0" if y_train_pred[i] == 1 else "Class Rest"
        actual_label = "Class 0" if y_train_binary[i] == 1 else "Class Rest"
    ...
    train_prec = precision_score(y_train_binary, y_train_pred, zero_division=0)
    train_rec = recall_score(y_train_binary, y_train_pred, zero_division=0)
    train_f1 = f1_score(y_train_binary, y_train_pred, zero_division=0)
    ...
    for i in range(min(10, len(y_test_pred))):
        pred_label = "Class 0" if y_test_pred[i] == 1 else "Class Rest"
        actual_label = "Class 0" if y_test_binary[i] == 1 else "Class Rest"
        print(f"  Sample {i}: Predicted={pred_label}, Actual={actual_label}, "
    ...
    print(f"  Class 0 samples: {sum(y_train_binary == 1)}")
    print(f"  Class Rest samples: {sum(y_train_binary == 0)}")
    print()
